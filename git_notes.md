...
**9. 建立远程库并上传文件**
  ...
  B. 在GITHUB上新建一个Repository，名字与本机已有repository相同。然后将二者建立关联：
     ...
     用命令$ git push -u origin master      (注：设置远程库分支master为默认分支后，将当前目录所有文件都推到远程库的指定分支master内)
...
**11. 克隆远程库**
克隆是将远程代码库的所有文件和版本历史复制到本地的操作，方便开发和协作。好处如下：
...
这个错误原因是你试图推送到一个不存在的 master 分支。许多新的仓库默认分支是 main 而不是 master。

解决方法一：
- `$ git branch -a` 查看所有分支，确认是否存在 master 分支。如有，则切换到该分支。如无，则创建并切换到 master 分支
`$ git checkout -b master` 创建并切换到 master 分支
`$ git push origin master` 推送到远程 master 分支

方法二：
使用现有的默认分支，例如 main来代替。
`$ git push origin main` 推送到远程 main 分支

这样即可解决 src refspec master does not match any 错误，并成功推送代码。
...
1、克隆步骤
获取仓库地址：在远程仓库页面上找到并复制仓库的URL地址。
使用命令克隆：在本地命令行中运行 git clone <URL> 命令，将URL替换为刚刚复制的地址。 

2、克隆完成文件位置
存储在当前目录下一个新文件夹中，文件夹名与远程仓库相同，包含所有文件和版本历史。 

3、命令行方式上传本地文件至远程仓库（推荐）
